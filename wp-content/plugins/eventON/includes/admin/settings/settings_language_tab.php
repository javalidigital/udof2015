	...
	function get_content(){
		...
		function language_variables_array(){
			...
			$output = array_merge($output, array(
				...
				array('type'=>'togheader','name'=>__('Calendar Header','eventon')),
					...
					,array(
						...
						'label'=>'Go to today','name'=>'evcal_lang_gototoday',
					),
					array('type'=>'togend'),
			...
			function _array_part_months(){
				$output = '';
				for($x=1; $x<13; $x++){
					$output['evcal_lang_'.$x] = (!empty($this->lang_options['evcal_lang_'.$x]))?  $this->lang_options['evcal_lang_'.$x]: $this->eventon_months[$x];
				}
				return $output;
			}
			function _array_part_3letter_months(){
				...
				for($x=1; $x<13; $x++){
					...
					$output['evo_lang_3Lm_'.$x] = (!empty($this->lang_options['evo_lang_3Lm_'.$x]))?  $this->lang_options['evo_lang_3Lm_'.$x]: $month_3l;
				}
				return $output;
			}
			function _array_part_1letter_months(){
				...
				for($x=1; $x<13; $x++){
					...
					$output['evo_lang_1Lm_'.$x] = (!empty($this->lang_options['evo_lang_1Lm_'.$x]))?  $this->lang_options['evo_lang_1Lm_'.$x]: $month_1l;
				}
				return $output;
			}
			function _array_part_day_names(){
				...
				for($x=1; $x<8; $x++){
					...
					$output['evcal_lang_day'.$x] = (!empty($this->lang_options['evcal_lang_day'.$x]))?  $this->lang_options['evcal_lang_day'.$x]: $default;
				}
				return $output;
			}
			function _array_part_3leter_day_names(){
				...
				for($x=1; $x<8; $x++){
					...
					$output['evo_lang_3Ld_'.$x] = (!empty($this->lang_options['evo_lang_3Ld_'.$x]))?  $this->lang_options['evo_lang_3Ld_'.$x]: $default;
				}
				return $output;
			}
			...
			function _array_part_custom_meta_field_names(){
				...
				for($x=1; $x<($cmd_verify+1); $x++){
					...
					$output['evcal_cmd_'.$x] = (!empty($this->lang_options['evcal_cmd_'.$x]))?  $this->lang_options['evcal_cmd_'.$x]: $default;
				}
				return $output;
			}
